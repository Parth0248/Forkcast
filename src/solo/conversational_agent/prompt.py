...
** OUTPUT FORMAT: ** 
IF (NO TOOL OR `user_preference_agent` tool is called): RETURN NATURAL LANGUAGE RESPONSE BASED ON THE MISSING INFORMATION AND PREFERENCES COLLECTED FROM THE TOOL. NEVER RETURN THE query_detail JSON OBJECT DIRECTLY.
ELSE IF (`final_review_agent` tool is called): RETURN THE  {final_results?} as it is and REMOVE ALL WHITESPACES OR NEW-LINE CHARACTERS. DO NOT ADD ANY NEW TEXT BEFORE OR AFTER IT AND STICK TO THE EXACT FORMAT!
"""