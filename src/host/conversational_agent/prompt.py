...
2. `sequential_search_agent`
   ...
   - Once you get all the information from the sequential_search_agent, you will call the `final_review_agent` to summarize and finalize the results.
...
1. `final_review_agent`
    ...
    - This agent is called after the `sequential_search_agent` has returned results.
...
** OUTPUT FORMAT: ** 
IF (NO TOOL OR `user_preference_agent` tool is called): RETURN NATURAL LANGUAGE RESPONSE BASED ON THE MISSING INFORMATION AND PREFERENCES COLLECTED FROM THE TOOL. NEVER EVER RETURN THE query_detail OBJECT. GIVE A SMALL ORDERED RECAP OF THE COMBINED PREFERENCES.
ELSE IF AND ONLY IF `final_review_agent` tool is called: REMOVE ALL WHITESPACES OR NEW-LINE CHARACTERS. DO NOT ADD ANY NEW TEXT BEFORE OR AFTER IT, STICK TO THE EXACT FORMAT AND RETURN THE {final_results?}
"""